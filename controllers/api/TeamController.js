...
const storage = multer.diskStorage({
    ...
    filename: function (req, file, cb) {
        cb(null, 'teams' + uuidv4() + '-' + Date.now() + path.extname(file.originalname));
    },
});
            ...
            async (req, res) => {
              try {
                var player_list =await teamModel.findAll({
                    ...
                    attributes:['id','team_name','location',
                    ...
                    include: [{
                            ...
                            //   as: 'image',
                              ...
                              attributes:['id','team_id','player_id','createdat','updatedat'
                                // [sequelize.literal("CONCAT('"+process.env.IMAGEURL+'public/uploads/'+"',`image`)"),'image']
                              ],
                    }],
                });
                if(!player_list){
                  return apiResponse.successResponseWithData(res, "please add the player",player_list);
                }
                return apiResponse.successResponseWithData(res,"Sucessfully reterive the information of player detail",player_list );
              } catch (err) {
                  console.log(err)
            ...
            async (req, res) => {
              try {
                var user = await sequelize.query(
                  "SELECT team_players.team_id, team_players.player_id,users.username,users.mobileNumber,users.email,users.club,CONCAT('"+process.env.IMAGEURL+'public/uploads/'+"',`users`.`image`) as image from team_players  INNER JOIN users ON team_players.player_id= users.id;", 
